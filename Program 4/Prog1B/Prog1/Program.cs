...
public class Program
{
    ...
    public static void Main(string[] args)
    {
       ...
       items.Sort(); // Sort - uses natural order  
        ...
        foreach (LibraryItem item in items)   // display list in ascending order by title
            Console.WriteLine($"{item}\n");
        Pause();

        // sort in descending order by copyright year
        items.Sort(new DescendingCopyrightYearOrder()); // Sort - uses specified Comparer class
        Console.Out.WriteLine("Sorted list (descending order by copyright year)");
        foreach (LibraryItem item in items)
            Console.WriteLine($"{item}\n");
        Pause();
